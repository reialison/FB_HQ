...
class Make{
    ...
    //////////////////////////////////////////////////////////
		...
		function branchesAjaxDrop($label=null,$nameID=null,$value=null,$params=array()){
	    	$CI =& get_instance();
	    	$str = "";
	    		$selectParams = $params;
	    		$selectParams['class'] = (isset($selectParams['class']) ?: "")." ajax-branches-drop selectpicker with-ajax";
	    		$selectParams['data-live-search'] = "true";
			$str .= $this->select($label,$nameID,null,$value,$selectParams);
	    	if ($this->returnitize($params)) return $str; else $this->code .= $str;
	    }
	   ...
	   function span($text=null,$params=array()){
	    ...
	    function AllBranchDrop($label=null,$nameID=null,$value=null,$placeholder=null,$params=array()){
	     ...
	     function branchesDrop($label=null,$nameID=null,$value=null,$placeholder=null,$params=array()){
	    	$CI =& get_instance();
	 		$CI->load->model('site/site_model');
	 		$CI->load->model('dine/setup_model');
	    	$str = "";
				$selectParams = $params;
				if(!isset($selectParams['return']))
					$selectParams['return'] = true;

				$results = $CI->setup_model->get_user_branch();
				if(empty($results)){
		...
		function checkboxDrop($label=null,$nameID=null,$value=null,$params=array(),$checked=false){
			$CI =& get_instance();
	 		$CI->load->model('site/site_model');
	    	$str = "";
				$selectParams = $params;
