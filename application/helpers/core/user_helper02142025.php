		...
		$CI->make->sBoxBody();
			...
			$CI->make->sDivRow();
			    $CI->make->sDivCol(4,'left',3);
			        $CI->make->button(fa('fa-save')." Save Details",array('id'=>'save-btn','class'=>'btn btn-success-600 radius-8 px-16 py-9'));
			    $CI->make->eDivCol();
			    $CI->make->sDivCol(2);
			        $CI->make->A(fa('fa-reply')." Go Back",base_url().'user',array('class'=>'btn btn-success-600 radius-8 px-16 py-9'));
			    $CI->make->eDivCol();
			$CI->make->eDivRow();
			$CI->make->eForm();
		$CI->make->eBoxBody();  
