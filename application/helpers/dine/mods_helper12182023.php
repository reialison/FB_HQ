		...
		$CI->make->sForm("mods/details_db",array('id'=>'details_form'));
			$CI->make->sDivRow();
				$CI->make->sDivCol(6);
					...
					$CI->make->input('Code','mod_code',iSetObj($mod,'mod_code'),'Type Code',array('class'=>'rOkay'));
					...
					$CI->make->sUl(array('class'=>'vertical-list','id'=>'modifier-list'));
						...
						foreach ($det as $res) {
							...
							$li = $CI->make->li(
				                ...
				                $CI->make->A(fa('fa-lg fa-times'),'#',array('return'=>true,'class'=>'del','id'=>'del-'.$res->id,'ref'=>$res->id,'ref2'=>$res->branch_code,'ref3'=>$res->mod_id)),
				                array('id'=>'li-'.$res->id)
				             );
						}
					$CI->make->eUl();
