		...
		$CI->make->sDivRow();
			$CI->make->sDivCol(3);
				...
				$CI->make->eBox();
						...
						// if(HAS_ITEM_RECIPE && $item_id){
						// 	$list[fa('fa-book').' Recipe'] = array('id'=>'recipe_link','class'=>'tab_link',
						// 														   'load'=>'items/recipe_load','href'=>'#details',	
						// 	                                        			   'style'=>'cursor:pointer;padding:10px;text-align:left;');
						// }
						// if($item_id){
...
function itemRecipeLoad($menu_id,$recipe=null,$det=array()){
	$CI =& get_instance();
	$CI->make->H(3,fa('fa fa-book').' Recipe',array('class'=>'page-header'));
	$CI->make->sDivRow();
		$CI->make->sDivCol(4);
			$CI->make->sForm('items/recipe_details_db',array('id'=>'recipe-details-form'));
		...
		$CI->make->sDivCol();
			$CI->make->sBox('info');
				// $CI->make->sForm("items/inventories",array('id'=>'general-form'));					
					$CI->make->sDivCol(2);
						$CI->make->QtyOnHand('Menu/Components','qty_on_hand',null,'- select-');
					$CI->make->eDivCol();					
					$CI->make->sDivCol(3);
						$CI->make->branchesDrop('Branch','branch_id',null,'- select branch -');
				...
				$CI->make->sBoxBody(array("style"=>"margin-top:35px"));
						...
						foreach ($records as $val) {
							$item_array = array($val['branch_name'],$val['code'],$val['name'],$val['cat_name'],$val['sub_cat_name'],$val['qoh'],);
							$rows[] = $item_array;
						}
		...
		$CI->make->sBoxBody();
			$CI->make->sDivRow();
				...
				$CI->make->sDivCol(3);
					...
					$range = $start." to ".$to;
					$CI->make->input('Date & Time Range','calendar_range',"",null,array('class'=>'rOkay daterangepicker','style'=>'position:initial;'),fa('fa-calendar'));
				$CI->make->eDivCol();
				$CI->make->sDivCol(3);
					$CI->make->branchesDrop('Branch','branch_id',null,'- select branch -',array('class'=>'rOkay'));
				$CI->make->eDivCol();
				$CI->make->sDivCol(3);
					$CI->make->itemAjaxDrop('Item','item-search',null,array('class'=>''));
				$CI->make->eDivCol();
				$CI->make->sDivCol(3);
					$CI->make->button(fa('fa-search').' Search',array('id'=>'search-btn','style'=>'margin-top:23px;margin-right:100px;'),'primary');
					// $CI->make->button(fa('fa-file-pdf-o').' PDF',array('id'=>'print-btn','style'=>'margin-top:23px;margin-right:10px;'),'success');
					// $CI->make->button(fa('fa-file-excel-o').' EXCEL',array('id'=>'excel-btn','style'=>'margin-top:23px;'),'success');
				$CI->make->eDivCol();
				$CI->make->eForm();
			$CI->make->eDivRow();	
			$CI->make->sDivRow();
