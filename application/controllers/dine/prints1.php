...
class Prints extends CI_Controller {
    ...
    #### FUNCTIONS
                ...
                if($sale->type_id == 10){
                    if($sale->trans_ref != "" && $sale->inactive == 0){                        
                        ...
                        if($sale->void_user_id){
                            $sales['cancel']['ids'][] = $sale->sales_id;
                            $sales['cancel']['orders'][$sale->sales_id] = $sale;
                    ...
                    $sales['void']['ids'][] = $sale->sales_id;
                    $sales['void']['orders'][$sale->sales_id] = $sale;
        ...
        public function menu_sales($ids=array(),$curr=false,$branch_code="",$brand="",$from="",$to=""){
            ...
            if(count($ids) > 0){
                ...
                $join['trans_sales'] = array('content'=>'trans_sales_menus.sales_id = trans_sales.sales_id AND trans_sales_menus.branch_code = trans_sales.branch_code && trans_sales_menus.pos_id = trans_sales.pos_id');
                $n_menu_res = array();
                // if($curr){
                //     $this->site_model->db = $this->load->database('default', TRUE);
                ...
                $this->site_model->db= $this->load->database('default', TRUE);
                if($branch_code != ""){
                    $args = array('trans_sales_menus.sales_id'=>$ids,'trans_sales_menus.branch_code'=>$branch_code,'trans_sales.inactive'=>0,'type_id'=>10);
                    if($brand != ''){
                        $args['trans_sales_menus.pos_id']=$brand;
                    }
                    $menu_res = $this->site_model->get_tbl('trans_sales_menus',$args,array(),$join,true,$select,array('sales_menu_id','branch_code','pos_id'));
                }else{
                    $menu_res = $this->site_model->get_tbl('trans_sales_menus',array('trans_sales_menus.sales_id'=>$ids,'trans_sales.inactive'=>0,'type_id'=>10),array(),$join,true,$select,array('sales_menu_id','branch_code','pos_id'));
                }                
                // echo $this->site_model->db->last_query(); die();
            ...
            if(count($ids) > 0){
                ...
                if($branch_code != ""){
                    ...
                    $item_res = $this->site_model->get_tbl('trans_sales_items',array('sales_id'=>$ids),array(),$join,true,$select,array('sales_item_id'));
                }
                 // echo $this->site_model->db->last_query();die();
            ...
            return array('gross'=>$menu_net_total+$total_md+$item_net_total+$total_smd,'menu_total'=>$menu_net_total,'total_qty'=>$menu_qty_total,'menus'=>$menus,'cats'=>$cats,'sub_cats'=>$sub_cats,'mods_total'=>$total_md,'mods'=>$mods,'free_menus'=>$free_menus,
                'items'=>$items,'item_total'=>$item_net_total,'item_total_qty'=>$item_qty_total,'submods_total'=>$total_smd,'submods'=>$sub_mods,'food_type'=>$food_types,'menu_trans_type'=>$menu_trans_type);
        }
        ...
        public function menu_top_sales($ids=array(),$curr=false,$branch_code="",$brand="",$from="",$to=""){
            ...
            if(count($ids) > 0){
                ...
                $join['(select * from trans_sales group by sales_id,branch_code,pos_id)trans_sales'] = array('content'=>'trans_sales_menus.sales_id = trans_sales.sales_id AND trans_sales_menus.branch_code = trans_sales.branch_code && trans_sales_menus.pos_id = trans_sales.pos_id');
                $n_menu_res = array();
                // if($curr){
                //     $this->site_model->db = $this->load->database('default', TRUE);
                //     $n_menu_res = $this->site_model->get_tbl('trans_sales_menus',array('sales_id'=>$ids,'trans_sales_menus.branch_code'=>$branch_code),array(),$join,true,$select);
                ...
                if($branch_code != ""){
                    ...
                    $menu_res = $this->site_model->get_tbl('( select * from trans_sales_menus group by branch_code, sales_menu_id,pos_id )trans_sales_menus',$args,array(),$join,true,$select,array('sales_menu_id','branch_code','pos_id'));
                }else{
                    $menu_res = $this->site_model->get_tbl('( select * from trans_sales_menus group by branch_code, sales_menu_id,pos_id )trans_sales_menus',array('trans_sales_menus.sales_id'=>$ids,'trans_sales.inactive'=>0,'menus.menu_sub_id !='=>7,'type_id'=>10),array(),$join,true,$select,array('sales_menu_id','branch_code','pos_id'));
                }                
                // echo $this->site_model->db->last_query(); die();
                // echo "<pre>",print_r($menu_res),"</pre>";die();
                foreach ($menu_res as $ms) {
         ...
         public function menu_toppings($ids=array(),$branch_code=""){
            ...
            if(count($ids) > 0){
                ...
                if($branch_code != ""){
                    ...
                    $menu_res = $this->site_model->get_tbl('( select * from trans_sales_menus group by branch_code,sales_menu_id,pos_id )trans_sales_menus',array('sales_id'=>$ids,'menus.menu_sub_id !='=>7),array(),$join,true,$select);
                }                
        ...
        public function charges_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                ...
                // }
                $join['trans_sales'] = array('content'=>'trans_sales_charges.sales_id = trans_sales.sales_id && trans_sales_charges.branch_code = trans_sales.branch_code && trans_sales_charges.pos_id = trans_sales.pos_id','mode'=>'left');

                $this->site_model->db = $this->load->database('default', TRUE);
                $cesults = $this->site_model->get_tbl('trans_sales_charges',$cargs,array(),$join);
                // echo $this->site_model->db->last_query();die();
        ...
        public function local_tax_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                ...
                $join['trans_sales'] = array('content'=>'trans_sales_local_tax.sales_id = trans_sales.sales_id && trans_sales_local_tax.branch_code = trans_sales.branch_code && trans_sales_local_tax.pos_id = trans_sales.pos_id','mode'=>'left');

                $this->site_model->db = $this->load->database('default', TRUE);
                $cesults = $this->site_model->get_tbl('trans_sales_local_tax',$cargs,array(),$join);
        ...
        public function tax_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                ...
                // }
                $join['trans_sales'] = array('content'=>'trans_sales_tax.sales_id = trans_sales.sales_id && trans_sales_tax.branch_code = trans_sales.branch_code && trans_sales_tax.pos_id = trans_sales.pos_id','mode'=>'left');

                $this->site_model->db = $this->load->database('default', TRUE);
                $cesults = $this->site_model->get_tbl('trans_sales_tax',$cargs,array(),$join);
                foreach ($cesults as $ces) {
                    if(!in_array($ces->sales_id, $ids_used)){
                        $ids_used[] = $ces->sales_id;
                    }
        ...
        public function no_tax_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                ...
                $join['trans_sales'] = array('content'=>'trans_sales_no_tax.sales_id = trans_sales.sales_id && trans_sales_no_tax.branch_code = trans_sales.branch_code && trans_sales_no_tax.pos_id = trans_sales.pos_id','mode'=>'left');
                $cesults = $this->site_model->get_tbl('trans_sales_no_tax',$cargs,array(),$join);
                foreach ($cesults as $ces) {
                    if(!in_array($ces->sales_id, $ids_used)){
                        $ids_used[] = $ces->sales_id;
                    }
        ...
        public function zero_rated_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                ...
                $join['trans_sales'] = array('content'=>'trans_sales_zero_rated.sales_id = trans_sales.sales_id && trans_sales_zero_rated.branch_code = trans_sales.branch_code && trans_sales_zero_rated.pos_id = trans_sales.pos_id','mode'=>'left');
                $cesults = $this->site_model->get_tbl('trans_sales_zero_rated',$cargs,array(),$join);
        ...
        public function old_grand_net_total($date="",$add_void=false,$branch_id=""){
            ...
            $this->site_model->db = $this->load->database('default', TRUE);
            $result = $this->site_model->get_tbl('( select * from trans_sales group by branch_code,sales_id,pos_id )trans_sales',$args,array(),null,true,'sum(trans_sales.total_amount) as total');
            // echo $this->site_model->db->last_query(); die();
            if(count($result) > 0){
                $old_grand_total += $result[0]->total;
            }
            ...
            $this->site_model->db = $this->load->database('default', TRUE);
            $result = $this->site_model->get_tbl('( select * from trans_sales_charges group by branch_code,sales_charge_id,pos_id )trans_sales_charges',$hargs,array(),$joinh,true,'sum(trans_sales_charges.amount) as total_charges');
            // echo $this->site_model->db->last_query();
            if(count($result) > 0){
            ...
            $this->site_model->db  = $this->load->database('default', TRUE);
            $result = $this->site_model->get_tbl('( select * from trans_sales_local_tax group by branch_code,sales_local_tax_id,pos_id )trans_sales_local_tax',$largs,array(),$joinl,true,'sum(trans_sales_local_tax.amount) as total_lt');
            if(count($result) > 0){
            ...
            if(MALL_ENABLED && MALL == 'megaworld'){
                ...
                $this->site_model->db  = $this->load->database('default', TRUE);
                $result = $this->site_model->get_tbl('( select * from trans_sales_payments group by branch_code,payment_id,pos_id )trans_sales_payments',$gcargs,array(),$joingc,true,'sum(trans_sales_payments.amount) as total_amount, sum(trans_sales_payments.to_pay) as total_topay');
                $gc_excess = 0;
                if(count($result) > 0){
    ...
    public function check_system_sales(){
        ...
        $print_date =$from. "  - ".$to;
        // $this->zread_sales_rep_backoffice($asjson,$args,false,$d,$print_date);
        $this->system_sales_sales_rep_backoffice_aurora($asjson,$args,false,$from,$print_date);
        // $this->system_sales_rep_jdmt_aurora($asjson,$args,false,$d,$print_date);

    }
    public function system_sales_sales_rep_backoffice_aurora($asJson=false,$args=array(),$return_print_str=false,$date,$print_date){
        ////hapchan
            ini_set('memory_limit', '-1');
            set_time_limit(3600);
    ...
    public function excel_system_sales($sales_id=null,$noPrint=true){
        ...
        $args["trans_sales.branch_code  = '".$branch_code."' "] = array('use'=>'where','val'=>null,'third'=>false);
            ...
            $printss = $this->system_sales_sales_rep_backoffice_aurora(false,$args,true,$from,$print_date);
    ...
    public function menuitem_sales_rep_gen()
    {
        ...
        $sales = $trans['sales'];
        // $curr = false;
        // $trans = $this->trans_sales_conso($args,$curr);
        // $sales = $trans['sales'];
        // $trans_menus = $this->menu_sales($sales['settled']['ids'],$curr,$terminal_id,$args_date);
        // $trans_charges = $this->charges_sales($sales['settled']['ids'],$curr,$terminal_id,$args_date);
        // $trans_discounts = $this->discounts_sales($sales['settled']['ids'],$curr,$terminal_id,$args_date);
        // $trans_local_tax = $this->local_tax_sales($sales['settled']['ids'],$curr,$terminal_id,$args_date);
        // $trans_tax = $this->tax_sales($sales['settled']['ids'],$curr,$terminal_id,$args_date);
        // $trans_no_tax = $this->no_tax_sales($sales['settled']['ids'],$curr,$terminal_id,$args_date);
        // $trans_zero_rated = $this->zero_rated_sales($sales['settled']['ids'],$curr,$terminal_id,$args_date);
        // $payments = $this->payment_sales($sales['settled']['ids'],$curr,$terminal_id,$args_date);
        $trans_menus = $this->menu_sales($sales['settled']['ids'],$curr,$post['branch_code']);
        $trans_charges = $this->charges_sales($sales['settled']['ids'],$curr,$post['branch_code']);
        $trans_discounts = $this->discounts_sales($sales['settled']['ids'],$curr,$post['branch_code']);
        $tax_disc = $trans_discounts['tax_disc_total'];
