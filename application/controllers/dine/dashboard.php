...
class Dashboard extends Prints {
    ...
    public function get_dashboard_details(){
        ...
        $group = 'trans_sales.branch_code desc';
        $join = array();
        ...
        echo json_encode(array('lastGT'=>$lastGT,'todaySales'=>$todaySales,'todayTransNo'=>$todayTransNo,'lsu'=>$lsu,'ts'=>$ts));
    }
    ...
    function chart_data()
    {
        ...
        $branch_code = $this->input->post("branch_code");
        $year = date("Y");
        // echo "<pre>",print_r($year_drop),"</pre>";die();
        $mdata = array();
        for($i=1;$i<=12;$i++){
