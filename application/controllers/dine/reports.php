...
class Reports extends CI_Controller {
    ...
    //#####
        ...
        public function activity_logs_rep_excel(){
            ...
            $sheet->getCell('E'.$rc)->setValue('Branch');
            ...
            #DETAILS
                ...
                if($branch_id != ''){
                    $args["logs.branch_code"] = $branch_id;
                }
                $logs = $this->reports_model->get_logs(null,$args);
                ...
                foreach ($logs as $res) {
                    ...
                    $sheet->getCell('E'.$rc)->setValue($res->branch_code);
                    $rc++;
                }
    ...
    //######  
        ...
        public function get_monthly_reports(){
            ...
            $terminal_id = $this->input->post('branch');
            $json = $this->input->post('json');
            ...
            update_load(10);
            $to_date = date('Y-m-d', strtotime($end_month . ' +1 day'));
            // echo $start_month."-".$open_time." ".$to_date."-".$close_time." ".$branch_id;die();
            $this->admin_model->set_temp_trans_sales($branch_id,$start_month." ".$open_time,$end_month." ".$close_time);
            sleep(1);
            $load = 10;
                    // echo $cmonth.'<br>';
            $month_array = array();
            // $start_month = date($year.'-05-08');
            while (strtotime($start_month) <= strtotime($end_month)) {
                ...
                $trans = $this->trans_sales($post['args'],false,$branch_id,$terminal_id);
                ...
                $trans_menus = $this->menu_sales($sales['settled']['ids'],false,$branch_id,$terminal_id);
                
                // }else{
                //   continue;
                // }

                $trans_charges = $this->charges_sales($sales['settled']['ids'],false,$branch_id,$terminal_id);
                $trans_discounts = $this->discounts_sales($sales['settled']['ids'],false, $branch_id,$terminal_id);
                $tax_disc = $trans_discounts['tax_disc_total'];
                $no_tax_disc = $trans_discounts['no_tax_disc_total'];
                $trans_local_tax = $this->local_tax_sales($sales['settled']['ids'],false, $branch_id,$terminal_id);
                $trans_tax = $this->tax_sales($sales['settled']['ids'],false, $branch_id,$terminal_id);
                $trans_no_tax = $this->no_tax_sales($sales['settled']['ids'],false, $branch_id,$terminal_id);
                $trans_zero_rated = $this->zero_rated_sales($sales['settled']['ids'],false, $branch_id,$terminal_id);
                ...
                $gt = $this->old_grand_net_total($pos_start,false,$branch_id);
        ...
        public function monthly_sales_excel($branch_id=null){
            //diretso excel na
            ...
            $sheet->getCell('Q'.$rn)->setValue('Charges');
            $sheet->mergeCells('R10:R11');
            $sheet->getCell('R'.$rn)->setValue('Net Sales');
            ...
            if($month_array){
                ...
                $sheet->getStyle('Q'.$rn)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
                $sheet->getCell('Q'.$rn)->setValue($total_charges);
                $sheet->getStyle('R'.$rn)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
                $sheet->getCell('R'.$rn)->setValue($net_sales_total);
            }
            if (ob_get_contents()) 
                ob_end_clean();
            header('Content-type: application/vnd.ms-excel');
        ...
        public function get_daily_reports(){
            ...
            $open_time = $details[0]->store_open;
            $close_time = $details[0]->store_close;
            $this->admin_model->set_temp_trans_sales($branch_id,$date." ".$open_time,$date." ".$close_time);
                    // echo $cmonth.'<br>';
            $month_array = array();
            ...
            // }
            // echo "<pre>",print_r($post),"</pre>";die();
            $trans = $this->trans_sales_no_inactive($post['args'],false,$branch_id,$post['brand']);
           ...
           // echo "<pre>",print_r($settled),"</pre>";die();
            ...
            // echo $gross .' '. $less_vat .' '. $nontaxable .' '. $zero_rated .' ' .$discounts;exit;
        ...
        public function trans_sales_no_inactive($args=array(),$curr=false,$branch_code="",$brand=""){
            $total_chit = 0;
            $chit = array();
            $n_results = array();
            // if($curr){
            ...
            $results = $this->cashier_model->get_trans_sales_no_inactive(null,$args,'',null,$branch_code,$brand);
            // echo $this->cashier_model->db->last_query();die();
            $orders = array();
            // echo "<pre>",print_r($results),"</pre>";
            // if(count($n_results) > 0){
            ...
            if(HIDECHIT){
                ...
                // }
                foreach ($results as $res) {
                    $res->sales_id = $res->pos_id.'-'.$res->sales_id;
                    if($res->type_id == 10){
                        // $this->site_model->db = $this->load->database('default', TRUE);
                        $where = array('sales_id'=>$res->sales_id,'branch_code'=>$branch_code);
                        $rest = $this->site_model->get_details($where,'trans_sales_payments');
        ...
        //         // }
        //         $this->cashier_model->db = $this->load->database('default', TRUE);
        //         $sales_discs = $this->cashier_model->get_trans_sales_discounts(null,array("trans_sales_discounts.sales_id"=>$ids,"trans_sales_discounts.branch_code"=>$branch_id));
        //         foreach ($sales_discs as $discs) {
        ...
        // public function local_tax_sales($ids=array(),$curr=false,$branch_code=""){
        //     $total_local_tax = 0;
        //     $ids_used = array();
        //     if(count($ids) > 0){
        ...
        // }
        // public function tax_sales($ids=array(),$curr=false,$branch_code=""){
        //     $total_tax = 0;
        //     $ids_used = array();
        //     if(count($ids) > 0){
        ...
        // }
        // public function no_tax_sales($ids=array(),$curr=false,$branch_code=""){
        //     $total_no_tax = 0;
        ...
        // }
        // public function zero_rated_sales($ids=array(),$curr=false,$branch_code=""){
        //     $total = 0;
        //     $ids_used = array();
        //     if(count($ids) > 0){
        ...
        //         $sales_menu_cat = $this->menu_model->get_menu_subcategories(null,true,$branch_code);
        ...
        //             $this->cashier_model->db = $this->load->database('default', TRUE);
        //             if($branch_code != null){
        //                 $menu_cat_sale_submods = $this->cashier_model->get_trans_sales_menu_submodifiers_prints(null,array("trans_sales_menu_submodifiers.sales_id"=>$ids,'trans_sales_menu_submodifiers.branch_code'=>$branch_code));
        //             }else{
        //                 $args_submod["trans_sales.type_id = 10"] = array('use'=>'where','val'=>null,'third'=>false);
        //                 $args_submod["trans_sales.inactive = 0"] = array('use'=>'where','val'=>null,'third'=>false);
        //                 $args_submod["trans_sales_menu_submodifiers.sales_id"] = array('use'=>'where_in','val'=>$ids,'third'=>false);
        //                 $menu_cat_sale_submods = $this->cashier_model->get_trans_sales_menu_submodifiers_prints(null,$args_submod);
        //             }
        //         // }else{
        ...
        //     #ITEMS
        //     if(count($ids) > 0){
        //         //wala na ko binago kasi di namn na nag bebenta ng items - jed
        ...
        //     }  
        //     // echo $menu_net_total;
        //     return array('gross'=>$menu_net_total+$total_md+$item_net_total+$total_smd,'menu_total'=>$menu_net_total,'total_qty'=>$menu_qty_total,'menus'=>$menus,'cats'=>$cats,'sub_cats'=>$sub_cats,'mods_total'=>$total_md,'mods'=>$mods,'free_menus'=>$free_menus,'item_total'=>$item_net_total,'submods_total'=>$total_smd);
        // }
        public function local_tax_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                ...
                $join['trans_sales'.$sess_id.' trans_sales use index(branch_code)'] = array('content'=>'trans_sales_local_tax.sales_id = trans_sales.sales_id && trans_sales_local_tax.branch_code = trans_sales.branch_code && trans_sales_local_tax.pos_id = trans_sales.pos_id','mode'=>'left');

                // $this->site_model->db = $this->load->database('default', TRUE);
                $cesults = $this->site_model->get_tbl('trans_sales'.$sess_id.'_local_tax trans_sales_local_tax use index(branch_code)',$cargs,array(),$join);
        ...
        public function tax_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                ...
                // if($curr){
                    ...
                    // $n_cesults = $this->site_model->get_tbl('trans_sales_tax',$cargs);
                // }
                $join['trans_sales'.$sess_id. ' trans_sales use index(branch_code)'] = array('content'=>'trans_sales_tax.sales_id = trans_sales.sales_id && trans_sales_tax.branch_code = trans_sales.branch_code && trans_sales_tax.pos_id = trans_sales.pos_id','mode'=>'left');

                // $this->site_model->db = $this->load->database('default', TRUE);
                $cesults = $this->site_model->get_tbl('trans_sales'.$sess_id.'_tax trans_sales_tax use index(branch_code)',$cargs,array(),$join);
                foreach ($cesults as $ces) {
                    if(!in_array($ces->sales_id, $ids_used)){
                        $ids_used[] = $ces->sales_id;
                    }
        ...
        public function no_tax_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                $cargs["concat(trans_sales_no_tax.pos_id,'-',trans_sales_no_tax.sales_id)"] = array('use'=>'where_in','val'=>$ids,'third'=>false);
                $cargs["trans_sales_no_tax.branch_code"] = $branch_code;
                ...
                $join['trans_sales'.$sess_id.' trans_sales use index(branch_code)'] = array('content'=>'trans_sales_no_tax.sales_id = trans_sales.sales_id && trans_sales_no_tax.branch_code = trans_sales.branch_code && trans_sales_no_tax.pos_id = trans_sales.pos_id','mode'=>'left');
                $cesults = $this->site_model->get_tbl('trans_sales'.$sess_id.'_no_tax trans_sales_no_tax use index(branch_code)',$cargs,array(),$join);
        ...
        public function zero_rated_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                 $cargs["concat(trans_sales_zero_rated.pos_id,'-',trans_sales_zero_rated.sales_id)"] = array('use'=>'where_in','val'=>$ids,'third'=>false);
                $cargs["trans_sales_zero_rated.branch_code"] = $branch_code;
                ...
                $join['trans_sales'.$sess_id.' trans_sales use index(branch_code)'] = array('content'=>'trans_sales_zero_rated.sales_id = trans_sales.sales_id && trans_sales_zero_rated.branch_code = trans_sales.branch_code && trans_sales_zero_rated.pos_id = trans_sales.pos_id','mode'=>'left');
                $cesults = $this->site_model->get_tbl('trans_sales'.$sess_id.'_zero_rated trans_sales_zero_rated use index(branch_code)',$cargs,array(),$join);
        ...
        public function menu_sales($ids=array(),$curr=false,$branch_code="",$brand="",$from="",$to=""){
            ...
            if(count($ids) > 0){
                ...
                $join = null;
                if($branch_code != ""){
                $join['menus use index(branch_code)'] = array('content'=>"trans_sales_menus.menu_id = menus.menu_id  AND menus.branch_code = trans_sales_menus.branch_code");
                }else{
                $join['menus use index(branch_code)'] = array('content'=>'trans_sales_menus.menu_id = menus.menu_id AND trans_sales_menus.branch_code = menus.branch_code');
                }
                $join['branch_details use index(branch_code)'] = array('content'=>'trans_sales_menus.branch_code = menus.branch_code AND trans_sales_menus.branch_code = branch_details.branch_code');

                $join['trans_sales'.$sess_id.' trans_sales use index(branch_code)'] = array('content'=>'trans_sales_menus.sales_id = trans_sales.sales_id AND trans_sales_menus.branch_code = trans_sales.branch_code && trans_sales_menus.pos_id = trans_sales.pos_id');
                $n_menu_res = array();
                // if($curr){
                //     $this->site_model->db = $this->load->database('default', TRUE);
                ...
                if($branch_code != ""){
                    ...
                    $menu_res = $this->site_model->get_tbl('trans_sales'.$sess_id.'_menus trans_sales_menus use index(branch_code)',$args,array(),$join,true,$select,array('sales_menu_id','branch_code','pos_id'));
                }else{
                    $args = array('trans_sales.inactive'=>0,'type_id'=>10);
                    $args["concat(trans_sales_menus.pos_id,'-',trans_sales_menus.sales_id)"] = array('use'=>'where_in','val'=>$ids,'third'=>false);
                    $menu_res = $this->site_model->get_tbl('trans_sales'.$sess_id.'_menus trans_sales_menus use index(branch_code)',$args,array('menus.menu_name'=>'asc'),$join,true,$select,array('sales_menu_id','branch_code','pos_id'));
                }                
                // echo $this->site_model->db->last_query(); die();
                // echo "<pre>",print_r($menu_res),"</pre>";die();
                foreach ($menu_res as $ms) {
    ...
    // echo 1;die();
        ...
        // public function charges_sales($ids=array(),$curr=false,$branch_code=""){
        //     $total_charges = 0;
        //     $charges = array();
        //     $ids_used = array();
        //     if(count($ids) > 0){
        ...
        public function charges_sales($ids=array(),$curr=false,$branch_code="",$brand="",$cargs=array()){
            ...
            if(count($ids) > 0){
                $cargs["concat(trans_sales_charges.pos_id,'-',trans_sales_charges.sales_id)"] = array('use'=>'where_in','val'=>$ids,'third'=>false);
                $cargs["trans_sales_charges.branch_code"] = $branch_code;
                ...
                //     $n_cesults = $this->site_model->get_tbl('trans_sales_charges',$cargs);
                // }
                $join['trans_sales'.$sess_id .' trans_sales use index(branch_code)'] = array('content'=>'trans_sales_charges.sales_id = trans_sales.sales_id && trans_sales_charges.branch_code = trans_sales.branch_code && trans_sales_charges.pos_id = trans_sales.pos_id','mode'=>'left');

                // $this->site_model->db = $this->load->database('default', TRUE);
                $cesults = $this->site_model->get_tbl('trans_sales'.$sess_id.'_charges trans_sales_charges use index(branch_code)',$cargs,array(),$join);
                // echo $this->site_model->db->last_query();die();
