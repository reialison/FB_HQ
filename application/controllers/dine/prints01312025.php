...
class Prints extends CI_Controller {
    ...
    // echo 1;die();
        ...
        public function menu_top_sales($ids=array(),$curr=false,$branch_code="",$brand="",$from="",$to=""){
         ...
         public function menu_toppings($args=array(),$branch_code=""){
            $this->site_model->db = $this->load->database('default', TRUE);
            ...
            $select = 'trans_sales_menus.*, sum(qty) as total_qty, sum(qty*price) as total_amount';
            $join = null;
            if($branch_code != ""){
            // $join['menus use index(branch_code)'] = array('content'=>"trans_sales_menus.menu_id = menus.menu_id  AND menus.branch_code = trans_sales_menus.branch_code");
            ...
            $join['trans_sales use index(branch_code)'] = array('content'=>"trans_sales_menus.sales_id = trans_sales.sales_id  AND trans_sales.branch_code = trans_sales_menus.branch_code");
            ...
            $this->site_model->db= $this->load->database('default', TRUE);
            if($branch_code != ""){
                $menu_res = $this->site_model->get_tbl('trans_sales_menus use index(branch_code)',$args,array(),$join,true,$select,array('menu_id'),10);
            }else{
                $menu_res = $this->site_model->get_tbl('trans_sales_menus use index(branch_code)',$args,array(),$join,true,$select,array('menu_id'),10);
            }                   
    ...
    public function check_zreads(){
        ...
        $from = date2SqlDateTime($dates." ".$open_time);
        $oa = date('a',strtotime($open_time));
        $ca = date('a',strtotime($close_time));
        $to = date2SqlDateTime($dates." ".$close_time);
        if($oa == $ca){
            $to = date('Y-m-d H:i:s',strtotime($to . "+1 days"));
        }

        $print_date = date('m/d/Y H:i:s',strtotime($from)). " - ".date('m/d/Y H:i:s',strtotime($to));
        ...
        $this->admin_model->set_temp_trans_sales($branch_code,$from,$to);
