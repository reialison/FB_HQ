...
class Menu extends CI_Controller {
    ...
    // echo "<pre>",print_r($post),"</pre>";die(); 
        ...
        if(count($items) > 0){
            ...
            foreach ($items as $res) {
                $link = $this->make->A(fa('fa-edit fa-lg').'  Edit',base_url().'menu/form/'.$res->menu_id.'/'.$res->branch_code,array('class'=>'btn btn-outline-success-600 radius-8 px-20 py-11 align-items-center gap-2','return'=>'true'));
                $json[$res->menu_id.'/'.$res->branch_code] = array(
                    ...
                    "sub"=>ucwords(strtolower($res->menu_cat_name)),   
    ...
    public function details_db(){
        ...
        $date_effective = date2Sql($date);
        $miaa = "";
        if($this->input->post('miaa_cat')){
            $miaa = $this->input->post('miaa_cat');
        }   

        if($this->input->post('menu_sub_id') == '' || $this->input->post('menu_sub_id') == 0){
            $menu_sub_id = null;
        }else{
        ...
        foreach ($branches as $value) {
            ...
            //    "kitchen_number"=>(int)$this->input->post('kitchen_number'),
               // "time_preparation"=>(int)$this->input->post('time_preparation'),
                ...
                "menu_other_desc"=>$this->input->post('menu_other_desc'),
            );
        }
    ...
    public function get_menu_categories($id=null,$asJson=true){
        ...
        $join["branch_details"] = array('content'=>"branch_details.branch_code = menu_categories.branch_code");
        ...
        $items = $this->site_model->get_tbl('menu_categories',$args,array(),$join,true,'menu_categories.*,branch_details.branch_name',null,$page['limit']);
        $json = array();
        if(count($items) > 0){
            foreach ($items as $res) {
                $link = $this->make->A(fa('fa-edit fa-lg').' Edit','#',array('class'=>'btn btn-outline-success-600 radius-8 px-20 py-11 align-items-center gap-2','id'=>'edit-'.$res->menu_cat_id.'-'.$res->branch_code,'ref'=>$res->menu_cat_id,'branch'=>$res->branch_code,'return'=>'true'));
                $json[$res->menu_cat_id.'/'.$res->branch_code] = array(
                    "id"=>$res->menu_cat_id,   
                    "title"=>ucwords(strtolower($res->menu_cat_name)),   
                    "date_reg"=>sql2Date($res->reg_date),
                    "branch_code"=>$res->branch_name,
    ...
    public function categories_form($ref=null,$branch=null){
        ...
        $this->data['add_css'] = 'css/wowdash.css';
    ...
    public function get_subcategories($id=null,$asJson=true){
        ...
        // $join = null;
        $join["branch_details"] = array('content'=>"branch_details.branch_code = menu_subcategories.branch_code");
        ...
        $items = $this->site_model->get_tbl('menu_subcategories',$args,array(),$join,true,'menu_subcategories.*,branch_details.branch_name',null,$page['limit']);
        $json = array();
        if(count($items) > 0){
            $ids = array();
            foreach ($items as $res) {
                $link = $this->make->A(fa('fa-edit fa-lg').' Edit','#',array('class'=>'btn btn-outline-success-600 radius-8 px-20 py-11 align-items-center gap-2','id'=>'edit-'.$res->menu_sub_cat_id.'-'.$res->branch_code,'ref'=>$res->menu_sub_cat_id,'branch'=>$res->branch_code,'return'=>'true'));
    ...
    public function get_subcategories_new($id=null,$asJson=true){
        ...
        $join["branch_details"] = array('content'=>"branch_details.branch_code = menu_subcategory.branch_code");
        ...
        $items = $this->site_model->get_tbl('menu_subcategory',$args,array(),$join,true,'menu_subcategory.*,menu_categories.menu_cat_name,branch_details.branch_name',null,$page['limit']);
        ...
        if(count($items) > 0){
            $ids = array();
            foreach ($items as $res) {
                $link = $this->make->A(fa('fa-edit fa-lg').' Edit','#',array('class'=>'btn btn-outline-success-600 radius-8 px-20 py-11 align-items-center gap-2','id'=>'edit-'.$res->menu_sub_id.'-'.$res->branch_code,'ref'=>$res->menu_sub_id,'branch'=>$res->branch_code,'return'=>'true'));
    ...
    public function menu_prices_db()
    {
        ...
        $branches_code = $this->items_model->get_branch_detail();
            foreach ($branches_code as $key => $b_code) {
                ...
                $mod_id = $this->menu_model->get_last_menu_prices($b_code->branch_code);
                $items['id'] =  $mod_id + 1;
                $id = $this->menu_model->add_menu_price($items);
                // $id +=1;
            }
                $det = $this->menu_model->get_menu_prices($items['id'],$items['trans_type']);
        } 
        // echo 'asdfasdf'; die();
    ...
    public function inventory(){
        ...
        $query = $this->menu_model->get_curr_menu_inv_and_locs();
        ...
        $data['code'] = menu_inventory_and_location_container($records, $loc_fields);
        // $data['page_title'] = fa('fa-random')." Items Inventory";
        $data['page_title'] = fa('fa-archive').'Menu Quantity On Hand';
    ...
    public function get_inv_move($id=null,$asJson=true){
        ...
        if(CONSOLIDATOR){
            if($terminal_id != null){
                $args['item_moves.pos_id'] = $terminal_id;
            }
        }else{
            // $args['item_moves.pos_id'] = TERMINAL_ID;
        }
        ...
        if(count($items) > 0){
            ...
            foreach ($items as $res2) {
                if(!in_array($res2->item_id, $recipe_list)){
                    $args2['item_moves.item_id'] = $res2->item_id;
                    $args3['item_moves.item_id'] = $res2->item_id;
                    ...
                    $recipe_list[] = $res2->item_id;
                     $this->make->sRow(array('class'=>'tdhd'));
                                    ...
                                    $this->make->td("");
                                    $c = "";
                                    if($res2->it_per_pack > 0){
                                        $c = $before_m_item/$res2->it_per_pack;
                                    }    
                                    $this->make->td($c,array('class'=>'text-right'));
                                $this->make->eRow();
                     foreach ($items as $res) {
                        if($res->item_id == $res2->item_id){
                            if($ctr == 1){
                     ...
                     $this->make->sRow(array('class'=>'tdhd'));
                        ...
                        $c = "";
                        if($res2->it_per_pack > 0){
                            $c = ($curr + $after_m_item)/$res2->it_per_pack;
                        } 
                        $this->make->td($c,array('class'=>'text-right'));
                    $this->make->eRow();
                }                    
