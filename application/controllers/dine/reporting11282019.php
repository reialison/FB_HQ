...
class Reporting extends Prints {
    ...
    public function menus_rep_gen(){
        ...
        update_load(10);
        $trans = $this->trans_sales($post['args'],$curr,$post['branch_code']);
        $sales = $trans['sales'];
        update_load(15);
    ...
    public function menus_rep_excel(){
        ...
        #GET VALUES
            start_load(0);
            $post = $this->set_post($_GET['calendar_range'],null,$_GET['branch_code']);
            $curr = true;
            update_load(10);
            $trans = $this->trans_sales($post['args'],$curr,$_GET['branch_code']);
            $sales = $trans['sales'];
            update_load(15);
            $trans_menus = $this->menu_sales($sales['settled']['ids'],$curr,$_GET['branch_code']);
            // echo "<pre>",print_r($post),"</pre>";die();
            update_load(20);
            $trans_charges = $this->charges_sales($sales['settled']['ids'],$curr,$_GET['branch_code']);
            update_load(25);
            $trans_discounts = $this->discounts_sales($sales['settled']['ids'],$curr,$_GET['branch_code']);
            update_load(30);
            $trans_local_tax = $this->local_tax_sales($sales['settled']['ids'],$curr,$_GET['branch_code']);
            update_load(35);
            $trans_tax = $this->tax_sales($sales['settled']['ids'],$curr,$_GET['branch_code']);
            update_load(40);
            $trans_no_tax = $this->no_tax_sales($sales['settled']['ids'],$curr,$_GET['branch_code']);
            update_load(45);
            $trans_zero_rated = $this->zero_rated_sales($sales['settled']['ids'],$curr,$_GET['branch_code']);
            update_load(50);
            $payments = $this->payment_sales($sales['settled']['ids'],$curr,$_GET['branch_code']);
            update_load(53);
            $gross = $trans_menus['gross']; 
            $net = $trans['net'];
            $charges = $trans_charges['total']; 
    ...
    public function sales_rep_gen()
    {
        ...
        $trans_ret = $this->menu_model->get_cat_sales_rep_retail($from, $to, $menu_cat_id, $branch_id);
        // echo $this->db->last_query(); die();             
        // echo "<pre>", print_r($trans), "</pre>"; die();
        $trans_count = count($trans) + count($trans_ret);
    ...
    public function menu_sales_rep_excel()
    {
        ...
        $trans = $this->menu_model->get_menu_sales_rep($from, $to, $menu_cat_id, $branch_id);
        $trans_ret = $this->menu_model->get_menu_sales_rep_retail($from, $to, "",$branch_id); 
        $trans_mod = $this->menu_model->get_mod_menu_sales_rep($from, $to, $menu_cat_id, $branch_id);
        $trans_payment = $this->menu_model->get_payment_date($from, $to);   
        ...
        $this->make->sDiv();
            $this->make->sTable(array("id"=>"main-tbl", 'class'=>'table reportTBL sortable'));
                ...
                foreach ($branch_details as $key => $vbranch) {
                    $this->make->sRow();
                        $this->make->td($vbranch->branch_code);
                        foreach ($param as $k => $v) {
                            $amt = 0;
    ...
    public function mom_rep_pdf()
    {
        ...
        foreach ($branch_details as $key => $vbranch) 
        {                           
            $pdf->Cell(43, 0, $vbranch->branch_code, '', 0, 'L');
            foreach ($param as $k => $v) {
                $amt = 0;
                $sales = $this->menu_model->get_monthly_sales($v["from"], $v["to"], $vbranch->branch_code);
                if(!empty($sales)){                                                       
    ...
    public function mom_rep_excel()
    {
        ...
        if(count($branch_details) > 0){            
            ...
            foreach ($branch_details as $key => $vbranch) 
            {                               
                $sheet->getCell('A'.$rc)->setValue($vbranch->branch_code);
                $sheet->getStyle('A'.$rc)->applyFromArray($styleTxt);
                $col="B";
                foreach ($param as $k => $v) {
                    $amt = 0;
            ...
            $this->make->sTable(array("id"=>"main-tbl", 'class'=>'table reportTBL sortable'));
                ...
                foreach ($branch_details as $key => $vbranch) {
                    $this->make->sRow();
                        ...
                        foreach ($param as $k => $v) {
                            $amt = 0;
                            $sales = $this->menu_model->get_yearly_sales($v["from"], $v["to"], $vbranch->branch_code);
                            if(!empty($sales)){                                
                                   $this->make->td(num($sales[0]->amt), array("style"=>"text-align:right"));
                                   $amt += $sales[0]->amt;
                            }else{
    ...
    public function voided_sales_rep_gen()
    {
        ...
        $trans = $this->menu_model->get_voided_cat_sales_rep($from, $to, $menu_cat_id, $branch_id);  
        $trans_ret = $this->menu_model->get_voided_cat_sales_rep_retail($from, $to, $menu_cat_id, $branch_id);
    ...
    public function voided_sales_rep_pdf()
    {
        ...
        $trans = $this->menu_model->get_voided_cat_sales_rep($from, $to, $menu_cat_id, $branch_id);
        $trans_ret = $this->menu_model->get_voided_cat_sales_rep_retail($from, $to, "", $branch_id);
