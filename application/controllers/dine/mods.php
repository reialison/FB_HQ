...
class Mods extends CI_Controller {
    ...
    public function get_modifiers($id=null,$asJson=true){
        ...
        if(count($items) > 0){
            ...
            foreach ($items as $res) {
                $link = $this->make->A(fa('fa-edit fa-lg').'  Edit',base_url().'mods/form/'.$res->mod_id."-".$res->branch_code,array('class'=>'btn btn-outline-success-600 radius-8 px-20 py-11 align-items-center gap-2','id'=>'edit-'.$res->mod_id,'ref'=>$res->mod_id,'ref2'=>$res->branch_code,'return'=>'true'));
    ...
    public function get_modifier_groups($id=null,$asJson=true){
        ...
        if(count($items) > 0){
            ...
            foreach ($items as $res) {
                ...
                $link = $this->make->A(fa('fa-edit fa-lg').'  Edit',base_url().'mods/group_form/'.$res->mod_group_id.'/'.$res->branch_code,array('class'=>'btn btn-outline-success-600 radius-8 px-20 py-11 align-items-center gap-2','id'=>'edit-'.$res->mod_group_id,'ref'=>$res->mod_group_id,'return'=>'true'));
     ...
     public function mod_prices_db()
    {
        ...
        $mod_trans_type = $this->mods_model->get_mod_prices_detail($this->input->post('trans_type'),$this->input->post('mod-id-hid'));
        if(empty($branch_selected)){
             $msg = "Error:No Branch Selected!";
             // site_alert($msg,'error');
         }else{
