...
class Site_model extends CI_Model{
   ...
   //   die();
	...
	public function get_data_graph($month,$year,$branch_code="",$user_id=""){
        // 		$this->db->select('sum(total_sales) as sales,store_zread.branch_code as branch_name');
        $this->db->select('sum(gross_sales) as sales,store_zread.branch_code as branch_name');
		// $this->db->from('trans_sales use index (branch_code)');
		$this->db->from(' store_zread');
		 // $this->db->join('branch_details', 'branch_details.branch_code = store_zread.branch_code','left');
		 // $this->db->join('user_branches', 'user_branches.branch_id = user_branches.branch_id','left');
		// $this->db->join('categories', 'items.cat_id = categories.cat_id','left');
		// $this->db->join('subcategories', 'items.subcat_id = subcategories.sub_cat_id','left');
		...
		{
			$this->db->where_in('store_zread.branch_code', $branch_code);			
		}
		// if(!empty($user_id))
		// {
		// 	$this->db->where('sys_user_id', $user_id);			
		// }
		// $this->db->where("void_ref is null");
		// $this->db->where('paid', 1);
		// $this->db->where('trans_sales.inactive', 0);
		$this->db->where('MONTH(trans_date) = '.$month);
		// $this->db->group_by('store_zread.branch_code');
		$this->db->limit(1000);
		$q = $this->db->get();
		// echo $this->db->last_query();die();
		return $q->result();
	...
	public function get_sales_per_store($year="",$branch_code=""){
		...
		if(!empty($branch_code))
		{
			$this->db->where_in('ts.branch_code', $branch_code);			
		}
