...
class Site_model extends CI_Model{
   ...
   //   die();
	...
	public function get_data_graph($month,$year,$branch_code=""){
		$this->db->select('*,sum(total_amount) as sales,branch_name');
		...
		$this->db->group_by('trans_sales.branch_code');
		$q = $this->db->get();
		// echo $this->db->last_query();die();
		return $q->result();
	}
	public function get_sales_per_store($year="",$branch_code=""){
		...
		$this->db->select('ts.branch_code,sum(ts.total_amount) as sales,branch_name');
