...
class Admin_model extends CI_Model{
	...
	function set_temp_trans_sales($branch_code='',$start_date='',$to_date=''){
		...
		$ts = '(select concat(sales_id,"-",pos_id) sales_id from trans_sales where branch_code="'.$branch_code.'" && datetime >= "'. $start_date .'" && datetime <= "'.$to_date.'") limit 1000000';
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.' select * from trans_sales where branch_code="'.$branch_code.'" && datetime >= "'. $start_date .'" && datetime <= "'.$to_date.'" limit 1000000');
// 			echo $this->db->last_query();die(); 
// 			echo "<pre>",print_r($session_id),"</pre>";die();
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_charges select * from trans_sales_charges where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_discounts select * from trans_sales_discounts where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_items select * from trans_sales_items where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_menu_modifiers select * from trans_sales_menu_modifiers where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_menu_submodifiers select * from trans_sales_menu_submodifiers where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_menus select * from trans_sales_menus where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_no_tax  select * from trans_sales_no_tax where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_payments select * from trans_sales_payments where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_tax select * from trans_sales_tax where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_zero_rated select * from trans_sales_zero_rated where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_local_tax select * from trans_sales_local_tax where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE trans_sales'.$session_id.'_loyalty_points select * from trans_sales_loyalty_points where branch_code = "'.$branch_code.'" && concat(sales_id,"-",pos_id) in '.$ts);
			$this->db->query('CREATE TEMPORARY TABLE reasons'.$session_id.' select * from reasons where branch_code = "'.$branch_code.'" && concat(trans_id,"-",pos_id) in '.$ts);
