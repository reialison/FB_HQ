...
class Menu_model extends CI_Model{
	...
	public function get_monthly_sales($from, $to, $branch_code="",$brand="")
	{
		...
		$this->db->select("branch_code, MONTH(trans_date) as month, sum(total_sales) as amt");
		...
		$this->db->where("DATE_FORMAT(trans_date, '%Y-%m-%d') >=", $from);
		$this->db->where("DATE_FORMAT(trans_date, '%Y-%m-%d') <=", $to);
		$this->db->group_by("branch_code, MONTH(trans_date), YEAR(trans_date)");
	...
	public function get_yearly_sales_dashboard($from, $to, $branch_code="")
	{
		...
		$this->db->select("YEAR(trans_date) as year, SUM(total_sales) as amt");
		// $this->db->from("trans_sales use index(branch_code)");
		$this->db->from("store_zread");
