...
$(document).ready(function(){
	<?php if($use_js == 'emaillistJS'): ?>
		$('#email-tbl').rTable({
			loadFrom	: 	 'email/get_email_settings',
			noEdit		: 	 true,
			add			: 	 function(){
								$.rPopForm({
									loadUrl : 'email/email_form',
									passTo	: 'email/email_form_db',
	...
	<?php elseif($use_js == 'recipeLoadJs'): ?>
		...
		$('#item-search').typeaheadmap({
			"source": function(search, process) {
				...
				var formData = 'search='+search+'&branch_code='+branch;
		...
		function get_item_details(v){
			...
			// alert(v);
			var branch = $('#branch_code').val();
			$.post(baseUrl+'mods/get_item_details/'+v+'/'+branch,function(data){
	...
	<?php elseif($use_js == 'modGroupFormJs'): ?>
		...
		function loader(btn){
			...
			$(tabPane).rLoad({url:baseUrl+loadUrl+'/'+mod_group_id+'/'+bcode});
		}
		function disEnbleTabs(id,enable){
			if(enable){
				$(id).parent().removeClass('disabled');
	...
	<?php elseif($use_js == 'groupRecipeLoadJs'): ?>
		...
		function add_to_group(id,text,branch_code){
			...
			var mod_text = text;
			$.post(baseUrl+'mods/group_modifiers_details_db','mod_group_id='+mod_group_id+'&mod_id='+mod_id+'&mod_text='+text+'&branch_code='+branch_code,function(data){
				// console.log(data);
				if(data.act == 'add'){
		...
		function remove_row(id,branch,mod_id){
			$('#del-'+id).click(function(){
				// alert(mod_id);
				$.post(baseUrl+'mods/remove_group_modifier','group_mod_id='+id+'&mod_id='+mod_id,function(data){
